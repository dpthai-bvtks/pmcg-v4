...
// ============================================================
        ...
        // ─── Chống double-click (Dùng withLock chuẩn hóa ở đầu app.js) ─
        ...
        window.isDroppedScheduleRow = isDroppedScheduleRow;
        window.normalizeScheduleRow = normalizeScheduleRow;
        ...
        function copyCurrentTabLink() {
            ...
            if (navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard.writeText(targetUrl).then(() => {
                    notify(`📋 Đã sao chép liên kết Tab: ${targetUrl}`);
                }).catch(() => {
        ...
        function saveProtocolFromForm() {
            ...
            if (!name) {
                notify('⚠️ Vui lòng nhập tên phác đồ điều trị!', 'warning');
            ...
            if (!selectedProcs.length) {
                notify('⚠️ Vui lòng chọn ít nhất 1 thủ thuật cho phác đồ!', 'warning');
                return;
            }
        ...
        function toggleContinuousProc(isChecked) {
            if (isChecked) {
                ...
                if (thMin && document.getElementById('proc-machine-time')) document.getElementById('proc-machine-time').value = thMin;
                if (thMax && document.getElementById('proc-machine-time-max')) document.getElementById('proc-machine-time-max').value = thMax;
            }
        }
    ...
    `;
        ...
        async function runExtraScheduling() {
            ...
            try {
                ...
                if (addedCount > 0) {
                    ...
                    callApi('saveSchedule', [dateVal, backendSched], 
                        () => { notify('☁️ Đã đồng bộ lịch bổ sung lên đám mây!', 'success'); },
                        (err) => { notify('⚠️ Lỗi lưu lịch bổ sung: ' + (err?.message || err), 'danger'); }
                    );
                }
        ...
        // ============================================================
        // 📤 XUẤT BÁO CÁO & TIMELINE Y TẾ (Đã module hóa sang js/modules/app-export-reports.js)
        // ============================================================
            ...
            setTimeout(() => {
                try {
                    ...
                    if (sched.length > 0) {
                        const backendSched = sched.map(x => scheduleRowToBackendArray(x, dateVal));
                        callApi('saveSchedule', [dateVal, backendSched], 
                            () => { notify('☁️ Đã đồng bộ lịch thứ 7 lên đám mây!', 'success'); },
                            (err) => { notify('⚠️ Lỗi lưu lịch thứ 7: ' + (err?.message || err), 'danger'); }
                        );
                    }
                } catch(err) {
                    btn.innerText = '▶ XẾP LỊCH THỨ 7'; btn.disabled = false;
        ...
        // ============================================================
        // ============================================================
        // ✅ KIỂM TRA LỖI HIS (Đã chuyển sang js/modules/app-error-checker.js)
        // ============================================================
